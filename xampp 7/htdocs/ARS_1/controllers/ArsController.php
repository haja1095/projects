...
class ArsController extends Controller
{
    ...
    public function actionIndex()
    {
       ...
       else
       {
           ...
           //$user_id = $this->Session->read("MstUser.id");
           $user_id =1;

           //$emp_id = $this->Session->read("MstUser.userref_id");
           $emp_id = 3318; //site_config--3277,3235 //3236--punch time have id
           ...
           /*------Total Worked Hours Calculate ------*/
           $LogHours = MstEmployee::getEmpLogHoursByMonthUsingShiftAttendance($mst_employee_id=$emp_id, $ars_month=$current_year, $ars_year=$current_month);
           ...
           if(!empty($for_display_all_employees) && !empty($user_id) || $user_id==1)
           {
               ...
               elseif(!empty($dept_hod)){ //--Department hod based employees list out
                  ...
                  $employees_list = ArrayHelper::map($employees_list, 'id' ,'emp_fname','dept_short_name');
                   ...
                   //Employee display..
                   $employees_list= (new \yii\db\Query())->select(['emp_fname'])->from('mst_employees')->where(['id'=>$emp_id,'status'=>1,'is_deleted'=>0])->one();
                   $full_access_right=0;
               }
           }
