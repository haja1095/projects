	   ...
	   <div>
		  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" align="right">
            <tr class="row_head" >
              ...
              <td width="20%"  class="text_field"><?php echo $paginator->sort( __('Client Name',true) ,'client_name');?></td>
			  <td width="30%"  class="text_field"><?php echo $paginator->sort( __('Client Address',true) ,'client_address');?></td>
			  <td width="20%"  class="text_field"><?php  echo $paginator->sort( __('Client Mobile',true) ,'client_mobile');?></td>
			...
			<?php
			 ...
			 if(isset($Clients) && !empty($Clients))
              ...
              <td class="text_field"><?php echo $Client['Client']['client_name'];?></td>
			  <td class="text_field"><?php echo $Client['Client']['client_address'];?></td>
			  <td class="text_field"><?php echo $Client['Client']['client_mobile'];?></td>
              <td class="text_field"><?php echo ($Client['Client']['status']==1?'Active':'Inactive');?></td>        
              <td class="action_field"  width="3%">
			   <div class="link_edit"><?php e($ajax->link( 'View',array('controller' => 'clients', 'action' =>'view', $Client['Client']['id']),array('title'=>'View','before'=>'clear_message();','loading'=>"javascript:onloading_indicator(true);",'complete'=>"javascript:onloading_indicator(false);",'update' => 'div_entry_form','escape' => false))); ?></div>						
			    </td>
			  <td class="action_field"  width="3%">
			    <div class="link_view"><?php e($ajax->link( 'Edit',array('controller' => 'clients', 'action' =>'edit', $Client['Client']['id']),array('title'=>'Edit','before'=>'clear_message();','loading'=>"javascript:onloading_indicator(true);",'complete'=>"javascript:onloading_indicator(false);",'update' => 'div_entry_form','escape' => false))); ?></div>						
			    </td>
			    <td class="action_field"  width="3%">
			   <div class="link_delete"> <?php e($ajax->link('Delete',array('controller'=> 'clients', 'action' =>'delete', $Client['Client']['id']),array('title'=>'Delete','before'=>'clear_message();','loading'=>"javascript:onloading_indicator(true);",'complete'=>"javascript:onloading_indicator(false);",'update' => 'div_entry_form','loaded'=>( $ajax-> remoteFunction(array('url' => array('controller' => 'clients', 'action' =>'index'), 'update' => 'indexpagging', 'frequency' => 5))),'escape' => false),'Do you want to Delete This Record?')); ?></div>						
