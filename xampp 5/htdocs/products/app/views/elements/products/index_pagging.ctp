	   ...
	   <div>
		  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" align="right">
            <tr class="row_head" >
              ...
              <td width="20%"  class="text_field"><?php echo $paginator->sort( __('Product Name',true) ,'product_name');?></td>
			  <td width="30%"  class="text_field"><?php echo $paginator->sort( __('Product Description',true) ,'product_desc');?></td>
			  <td width="20%"  class="text_field"><?php  echo $paginator->sort( __('Product Version',true) ,'product_version');?></td>
			...
			<?php
			 ...
			 if(isset($Products) && !empty($Products))
              ...
              <td class="text_field"><?php echo $Product['Product']['product_name'];?></td>
			  <td class="text_field"><?php echo $Product['Product']['product_desc'];?></td>
			  <td class="text_field"><?php echo $Product['Product']['product_version'];?></td>
              <td class="text_field"><?php echo ($Product['Product']['status']==1?'Active':'Inactive');?></td>        
              <td class="action_field"  width="3%">
			   <div class="link_edit"><?php e($ajax->link( 'View',array('controller' => 'products', 'action' =>'view', $Product['Product']['id']),array('title'=>'View','before'=>'clear_message();','loading'=>"javascript:onloading_indicator(true);",'complete'=>"javascript:onloading_indicator(false);",'update' => 'div_entry_form','escape' => false))); ?></div>						
			    </td>
			  <td class="action_field"  width="3%">
			    <div class="link_view"><?php e($ajax->link( 'Edit',array('controller' => 'products', 'action' =>'edit', $Product['Product']['id']),array('title'=>'Edit','before'=>'clear_message();','loading'=>"javascript:onloading_indicator(true);",'complete'=>"javascript:onloading_indicator(false);",'update' => 'div_entry_form','escape' => false))); ?></div>						
			    </td>
			    <td class="action_field"  width="3%">
			   <div class="link_delete"> <?php e($ajax->link('Delete',array('controller'=> 'products', 'action' =>'delete', $Product['Product']['id']),array('title'=>'Delete','before'=>'clear_message();','loading'=>"javascript:onloading_indicator(true);",'complete'=>"javascript:onloading_indicator(false);",'update' => 'div_entry_form','loaded'=>( $ajax-> remoteFunction(array('url' => array('controller' => 'products', 'action' =>'index'), 'update' => 'indexpagging', 'frequency' => 5))),'escape' => false),'Do you want to Delete This Record?')); ?></div>						
