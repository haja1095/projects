   <?php echo $form->create('ReleaseMode', array('action' => 'edit'));?>
...
<?php 
  ...
  <tr class="add_row0">
    <td width="40%" class="left_condent"><font color="#F41B0F">*</font><?php echo __('Release Name');?></td>
    <td width="1%" class="center_condent">:</td>
    <td width="59%" class="right_condent"><?php echo $form->input('release_name',array('error'=>false,'div'=>false,'class'=>'text_input','label'=>false));?></td>
  </tr>
  <tr class="add_row1">
  ...
  <tr class="add_row0">
	...
	<td class="center_condent">&nbsp; </td>
     <td class="right_condent"><?php echo $ajax->submit('Update', array('div' => false, 'type'=>'submit','class'=>'update-btn' , 'url' => array('controller'=> 'release_modes', 'action' => 'edit',$this->data['ReleaseMode']['id']), 'before'=>'clear_message();','loading'=>"javascript:onloading_indicator(true);",'complete'=>"javascript:onloading_indicator(false);", 'update'=> 'div_entry_form','loaded'=>( $ajax-> remoteFunction(array('url' => array('controller' => 'release_modes', 'action' =>'index'), 'update' => 'indexpagging', 'frequency' => 5))) )); ?></td>
  </tr>
</table>
<?php echo $form->end();?>